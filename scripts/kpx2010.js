...
function KPX2010(P, degree=14) {
  ...
  for(var m=0; m<n; m++) {
    console.log(m);
    eventQueue.push(new GraphEvent("line", "highlight", [], -1 )); // clear any highlighted edges
    eventQueue.push(new GraphEvent("point", "highlight", P[m], 0));
    ...
    for( var j=0; j<N.length; j++ ) {
      if( isFinite(N[j]) ) {
        eventQueue.push(new GraphEvent("line", "highlight", [P[m],P[N[j]]], 0));
        var theta = angle(
          refPoint,
          pointSet[m],
          pointSet[N[j]]
